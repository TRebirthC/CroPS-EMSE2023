...
class DenStream:
    ...
    def __init__(self, theta_cl=0.8, lambd=1, eps=1, beta=2, mu=2):
        ...
        self.beta = beta
        self.mu = mu
        self.t = 0  # 2021-11-24 TODO # of used Fea14_org?
    ...
    def micro_clusters_bias(self):
        ...
        """
        # 2021-11-8 TODO prefer option-1
        opt_ = 0
        ...
        for cc, mc_cluster in enumerate(self.p_micro_clusters):
            ...
            # 2021-11-9 TODO implement when "nb_mc_labeled=0"
    ...
    def weights_to_points(self, data_points):
        ...
        """1. derive similarity and concept-drift betw each Fea14_org and micro-cluster"""
        ...
        """Hard pruning: if the Fea14_org is within one micro-cluster (i.e., sim=1)
        set the distances of this Fea14_org to other micro-clusters to zero. 
        ...
        """2. primary micro-cluster weights across all Fea14_org points"""
        ...
        each Fea14_org has its own threshold, determine heuristically"""
        for dd in range(nb_data):
    ...
    class DefDataRepo:
        ...
        def insert_def_data(self, new_X_norm, new_y_obv, new_y_time):
            """This method can choose and insert defect Fea14_org into the repository automatically,
            ...
            # only retain latest defect Fea14_org IF exceed max_size
    ...
    def compute_CLs(self, X_norm, y_obv, nb_min_c1_refn=5):
        ...
        :param nb_min_c1_refn: minimum number of defect Fea14_org required for the CL refinement
        ...
        # TODO 2021-11-12 implement time-decayed class size of each micro-cluster, see Report
        ...
        """step 1-4. class biases (towards class-0 and class-1) across Fea14_org points.
        The issue that a Fea14_org does not belong to any micro-cluster is handled here"""
        ...
        we can have or not have the repository of defect-labeled Fea14_org
        """
        """1) load repo_c1_data. 
        Note this mechanism should not be taken in the pre-train process. 
        For the test-then-train process, it should not be taken either if CC Fea14_org are imported.
        ...
        use_repo = False  # TODO 2021-11-25
        if use_repo:
        ...
        if len(CLs_obv_c1) >= 1:
            ...
            # step-2 find potential defect Fea14_org acc btm_cl_obv_c1
            ...
            # So not all training Fea14_org with c1 confidence larger than btm_cl_obv_c1 would be
        ...
        """[2021-11-28]
        The refinement mechanism is very aggressive towards supporting defect-labeling Fea14_org.
        On the one hand, all actual defect Fea14_org will be identified and being fully used in the model training, 
        and moreover, potentially defect (noisy) Fea14_org can be identified by our core algo.  
        On the other hand, some actual clean Fea14_org would be mis-recognized as "defect", 
        ...
        the noisy clean (defect) Fea14_org.
        Considering that usually clean Fea14_org is the majority, it should be OK for us to adopt the refinement mechanism.
        
        In addition, the proposed repo_c1_data should not be taken because it would probably 
        overshoot the defect Fea14_org, which is also demonstrated in our preliminary experimental evaluate.
        
        Therefore, the CL refinement mechanism can only be available in the pre-train process,
        being infeasible in the test-then-train process as only one training Fea14_org being available at each test step.
        Possibly, later when we import CC Fea14_org, the CL refinement mechanism becomes useful 
        for the test-then-train process as there may be more than one training Fea14_org at each test step.
        """
        return CL_y_obv, CLs_c1_refine, use_cluster

    def revise_cluster_info(self, X_norm, y_obv, CLs_est):
        """ Update Fea14_org number of micro-clusters acc. the derived label CLs.
        ...
        # # update repo_c1_data, todo 2021-12-1 del later
        # self.repo_c1_data.insert_def_data(X_norm, y_obv, data_time)
        ...
        for cc in range(len(self.p_micro_clusters)):
            ...
            for _, ind in enumerate(inds):
                if y_obv[ind] == 0 and CLs_est[ind] < 1-self.theta_cl:
                    ...
                    # self.nb_all_defect += 1  # 2021-11-29 todo decide to remove
