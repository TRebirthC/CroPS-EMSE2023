"""
This file is related to the base JIT-SDP model ODaSC.
...
"""Liyan commented the below off on 2021-7-22"""
...
class OzaBaggingClassifier_OOC(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    version can effectively handle Fea14_org streams.
    ...
    drawn by the binomial distribution. Since we can consider the Fea14_org stream
    ...
    # >>> from skmultiflow.Fea14_org import SEAGenerator
    ...
    def partial_fit(self, X, y, CLs_np, classes=None, sample_weight=None):
        ...
        2021-12-14  Shuxian finds a bug making nb_tree useless as all trees employ exactly the same training Fea14_org
        """
        if self.classes is None:
            if classes is None:
                raise ValueError("The first partial_fit call should pass all the classes.")
        ...
        r, _ = get_dimensions(X)  # r: Fea14_org size
        y = y.reshape(r, )
        CLs_np = CLs_np.reshape(r, )

        for j in range(r):  # for each Fea14_org point
            ...
            for i in range(self.actual_n_estimators):  # all base learners
                ...
                if k > 0:
                    for b in range(k):
                        self.ensemble[i].partial_fit([X[j]], [y[j]], classes, None)
        return self
    ...
    def predict(self, X):
        """ Predict classes for the passed Fea14_org.
